...
class profile extends US_Controller
{
    ...
    {
        $this->form_validation->set_rules('old_password', 'Current Password', 'trim|required|max_length[10]');
        $this->form_validation->set_rules('new_password', 'New Password', 'trim|required|max_length[10]');
        $this->form_validation->set_rules('retyped_password', 'Retyped Password', 'trim|required|max_length[10]');
        ...
        $password_exists = $this->M_Employees->check_password(md5($old_password), $user_id);
        ...
        $data_insert = array(
            'employee_password' => md5($new_password),
        );

        $changePassword = $this->Common_model->update_table($data_insert, $user_id, 'ci_employees', 'employee_id');
    ...
    public function list_logs_json()
    {
        ...
        if ($user_id > 0) :
            ...
            foreach ($records['data']   as $row) {
                ...
                $data[] = array(
                    ++$i,
                    date('Y-m-d ', strtotime($row['login_time'])) . ' @ ' . date('H:i ', strtotime($row['login_time'])),
                    $row['login_os'],
                    $row['login_browser'],
                    $row['login_ip'],
                    $row['login_device']
