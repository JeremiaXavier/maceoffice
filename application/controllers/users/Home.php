...
class home extends US_Controller
{
    ...
    {
        $data = array();
        $this->template->users_views('users/site_settings/index', $data);
    }
    ...
    {
        $data = $this->data;
        $this->template->users_views('users/messages', $data);
    }
    ...
    public function recent_activity()
    {
        ...
        foreach ($records['activity']   as $row) {
            $records['activity'][$i++]->created_time = date('h:i:s', strtotime($row->created_at));
        }
        $this->response(200, $records);
    }
    ...
    public function visitors_traffic()
    {
        ...
        $records['traffic'] = $this->Dashboard_model->get_visitors_traffic();
    ...
    public function dashboard_contact_messages()
    {
        ...
        foreach ($records['data']   as $row) {
            ...
            $data[] = array(
                ...
                $row->email_address,
                $row->created_at,
                '<span class="badge bg-' . $status_bg . '">' . $row->status_badge . '</span>',
    ...
    public function visitors_list_json()
    {
        ...
        foreach ($records['data']   as $row) {
            ...
            $data[] = array(
                ++$i,
                $row->visited_date . ' @ ' . $row->visited_time,
    ...
    public function visitors_list()
    {
        $data = $this->data;
        //exit();
        $this->template->users_views('admin/visitors_list', $data);
    }
}
